...
def save_phase_b_report(swap_results: list[JudgeResult], kappa: float,
                        bias: dict, judge_labels: list[int],
...
if __name__ == "__main__":
    ...
    BAD_BASELINE = "Tôi không có thông tin về chính sách này."
    swap_results = []
    judge_labels = []
    for item in human_data:
        jr = swap_and_average(item["question"], item["model_answer"], BAD_BASELINE)
        swap_results.append(jr)
        if jr.final_winner == "A":
            judge_labels.append(1)
        elif jr.final_winner == "B":
            judge_labels.append(0)
        else:
            judge_labels.append(item["human_label"])
    ...
    print(f"\nBias report: {bias}")
    save_phase_b_report(swap_results, kappa, bias, judge_labels)
